...
from apps.anime.models import Anime, AnimeFormat, ReleaseSeason, Genre, AnimeStatus, Studio
from apps.anime.forms import AddAnimeForm
...
# Функции-хелперы
def get_anime_list(request, data, context=None, per_page=2):
...
def anime_genre(request, genre_slug):
    ...
    data = ensure_not_empty(
        Anime.objects.published().with_genres().by_genre(genre=genre.slug)
    )
...
def anime_season(request, season=None, year=None):
    data = ensure_not_empty(
        Anime.objects.published().with_genres().by_season(season=season, year=year)
    )
